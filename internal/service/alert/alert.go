...
// 本文件职责：每轮检查四类风险并通知用户——
//
//	费率反转（持仓币对费率 < 0，critical）、ADL 排名 ≥ 4（critical）、
//	标记价距强平价 < 10%（critical）、双所资金偏离 1:N 超阈值（warning，只提醒不自动调仓）。
//
// 阅读目的：看 fire() 一个函数就够了——所有预警统一走“4 小时去重 → 写 alert_log
// → 发邮件 → 同步写一条 trade_log”的管线；返回的预警记录同时是 autotrade
// fast sell 的触发信号。
package alert
